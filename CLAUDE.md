...
`dns/gen_knot_conf.py` regenerates `dns/files/knot.conf` from two hardcoded lists in the script: `PRIMARY` (user-owned zones we master) and `OFB_SLAVES` (the residual ofb-mastered zones still pointed at us, audited by parent NS + apex NS resolution against this host's IPs). Run the script after editing either list. To audit whether an ofb-mastered zone *needs* to be in `OFB_SLAVES`, query its parent registry NS and its apex NS, resolve each NS hostname to an IP, and check whether any resolve to `104.200.25.248` or this host's v6 addresses; if none do, the zone can safely be dropped from `OFB_SLAVES` (resolvers won't ask us). The list will eventually go empty as zone owners migrate apex NS off `ns2.ofb.net` and registrar NS off `egnor-2020.ofb.net`; at that point the ofb_slave/ofb_master/ofb_notify blocks come out too.
...
The Knot config in `dns/setup.py` also stops `named.service` (BIND9), since the two can't share port 53. Before the cutover, BIND was running here as `ns2.ofb.net` — secondary for ofb.net's then ~124 zones, slaving from `104.197.242.163`. Knot's `knot.conf` initially declared the same set as `template: ofb_slave`, taking over the secondary role wholesale; the list has since been pared down to just the residual zones still pointed at us (see `OFB_SLAVES` in `dns/gen_knot_conf.py`).
