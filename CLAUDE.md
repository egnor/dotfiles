...
- `netdata/` — Netdata config. Parent vs child role picked by hostname (`egnor-2020` is the parent; everywhere else is a child streaming up to it). On Linux, manages `netdata.conf` + `stream.conf` (config dir `/etc/netdata`), plus the `go.d/` and `health.d/` overrides, and installs `smartmontools` so the go.d `smartctl` collector reports SMART disk health on physical hosts; alerts are evaluated on the parent (`files.parent/health.d/`) since children run with `[health] enabled = no`. Fleet-wide alerts beyond SMART: failed systemd service units (`health.d/systemdunits.conf` — netdata ships this template disabled via a match-nothing `unit_name=!*` selector; our same-named file replaces the stock one and enables it) and unattended-upgrades freshness (`go.d/filecheck.conf` on parent+children watches `/var/lib/apt/periodic/upgrade-stamp`, touched only on successful u-u runs; `health.d/apt_upgrade.conf` alerts on stale or never-created stamps — needed because `apt.systemd.daily` swallows u-u's exit code, so u-u failures never fail the systemd unit). External endpoints (our sites plus Shopify-hosted `shop.seventeengames.com`) are probed from the parent by `go.d/httpcheck.conf` — plain 200-OK jobs and redirect-assertion jobs (`not_follow_redirects` + `status_accepted` + `header_match` on `Location`, so a changed redirect target alerts until the config is updated to match) — with cert expiry covered separately by `go.d/x509check.conf`, one job per (host, port). This replaces what used to be a handful of uptimerobot monitors emailing on failure; see `deadman/` for the part uptimerobot did that netdata structurally cannot do for itself.
- `deadman/` — host-specific (`Hostname == "egnor-2020"`). Two dead-man's switches for the things that cannot report their own death. Everything else in this repo alerts *from* egnor-2020, so egnor-2020's own death is the one outage it cannot report: netdata, the alert evaluator, postfix, and several monitored sites all go at once. Both tickle external healthchecks.io checks, which notify the same `ntfy.sh/eacs-alerts` topic netdata uses, so everything still lands in one place. Each reads its secret from an out-of-band `/etc/default/<name>` file (forgeable-ping secrets, like the ntfy token); if unset the script exits non-zero on purpose, so netdata's `systemdunits` alert catches a switch that was never configured. **`netdata-deadman.timer`** fires `/usr/local/sbin/netdata-deadman` every 5 minutes; the script pings `HC_PING_URL`, but only after local netdata answers `http://127.0.0.1:19998/api/v1/info` — so it covers netdata-dead and netdata-wedged, not just host-down (on a bad netdata it pings `<url>/fail` for an immediate alert instead of waiting out the grace period). **`mail-deadman.timer`** fires `/usr/local/sbin/mail-deadman` every 30 minutes; there the ping *is* a mail message — `sendmail` to `HC_MAIL_ADDRESS` (`<uuid>@hc-ping.com`, healthchecks.io's email-ping address; that domain has no MX, so delivery falls back to its A record) — so registering the ping requires the whole outbound path: postfix running, the chrooted resolver answering, DNS, outbound :25, and the queue actually moving. It exists because queue depth is not a usable signal on this host (a permanent backlog of undeliverable spam and backscatter ages out on its own, so any threshold is wrong by next month) and because the 2026-08-27 chroot-resolver outage went unnoticed for 23 hours. The two are deliberately separate checks rather than one: both silent means the host is gone, mail alone means the host is fine and delivery specifically is broken.
...
`deadman/` needs one-time out-of-band setup; `pyinfra` deploys the timers but cannot create the account or the checks. There are two independent checks — the netdata switch below, and the outbound-mail switch after it.

### netdata switch

1. At [healthchecks.io](https://healthchecks.io), create a check named `eacs.io netdata` with **period 5m** and **grace 10m**. healthchecks.io marks a check late at `period` and down at `period + grace`, so that alerts ~15 min after the last good ping: long enough to ride out a clean reboot (the timer is `Persistent=true`, so a reboot pings as soon as it is up), short enough that a reboot which *doesn't* come back is caught fast. Brief "late" blips between pings are normal and don't notify — that's what the grace period is for.
2. In that project's **Integrations**, add **ntfy**: topic `eacs-alerts`, server `https://ntfy.sh`, access token with publish rights on that topic. Priority **max** for down, **default** for up. Max (not high) because this is the one alert that arrives when nothing else can — the host, netdata, and mail are all gone — and on ntfy max priority bypasses Do Not Disturb. That is affordable here because reboots on this host are deliberate, not automatic (`tweaks/files/apt-auto-upgrades` sets no `Unattended-Upgrade::Automatic-Reboot`), so false alarms are rare. It also lines up with netdata's own mapping in `health_alarm_notify.conf`, where CRITICAL sends urgent. Generate a fresh token at ntfy.sh rather than reusing the `NTFY_ACCESS_TOKEN` from `/etc/netdata/health_alarm_notify_secrets.conf` — same topic either way, but separately revocable. Then attach the integration to the check.
3. On egnor-2020, drop the ping URL where the script will find it:

   ```
   printf 'HC_PING_URL=https://hc-ping.com/<uuid>\n' | sudo tee /etc/default/netdata-deadman
   sudo chmod 600 /etc/default/netdata-deadman
   ```

   The file is `.`-sourced by the script, so it must be a shell assignment (`HC_PING_URL=<url>`), not a bare URL on a line by itself. No trailing slash and no quotes needed — the script appends `/fail` to it directly.

4. `pyinfra eacs.io deploy.py`, then force one firing and confirm the check goes green:

   ```
   sudo systemctl start netdata-deadman.service
   systemctl status netdata-deadman.service
   systemctl list-timers netdata-deadman.timer
   ```
...
Same shape, different failure domain — and note this one's probe is a mail message, so there is no ping URL to configure, only an address to send to.

1. At healthchecks.io, create a second check named `eacs.io mail` with **period 1h** and **grace 1h**. Slower than the netdata switch on purpose: mail is bursty, and a greylist or a ten-minute Google hiccup should not page anyone. Worst case is ~2h to alert, which is still two orders of magnitude better than the 23 hours the 2026-08-27 outage actually took to notice.
2. Attach the same **ntfy** integration (step 2 above) to this check. A separate check, but the same topic — everything still lands in one place.
3. On that check's page, find the **email ping address** — `<uuid>@hc-ping.com`. Note `hc-ping.com` publishes no MX record, so delivery falls back to its A records per RFC 5321 §5.1; postfix does that natively and needs no special config. Then:

   ```
   printf 'HC_MAIL_ADDRESS=<uuid>@hc-ping.com\n' | sudo tee /etc/default/mail-deadman
   sudo chmod 600 /etc/default/mail-deadman
   ```

   Like the netdata switch, the file is `.`-sourced, so it must be a shell assignment. `MAIL_DEADMAN_FROM` may also be set there; it defaults to `mail-deadman@eacs.io`.
4. `pyinfra eacs.io deploy.py`, then force one firing and confirm the probe leaves the queue and the check goes green:

   ```
   sudo systemctl start mail-deadman.service
   journalctl -u mail-deadman.service -n 20
   sudo grep hc-ping /var/log/mail.log | tail -3      # want status=sent
   systemctl list-timers mail-deadman.timer
   ```

Two things worth knowing. `sendmail` injects into the maildrop queue and returns success even when postfix is stopped, so a green `mail-deadman.service` does **not** mean the mail was delivered — only arrival registers the ping, which is exactly the property that makes this a dead-man's switch rather than a liveness check. And the UUID lands in `/var/log/mail.log` as the recipient of every probe, plus its rotated archives; that is accepted rather than solved, since the log is `0640 root:adm` on the same host that stores the secret, and a leaked ping address only lets someone keep this one alert quiet.

Do not fold this into the netdata check. The pair is diagnostic: both silent means the host is gone, mail alone means the host is fine and delivery specifically is broken. Nor is this a job for a queue-depth alert — this host carries a permanent backlog of undeliverable spam and backscatter that ages out on its own, so depth and even deferral count are uninformative and any threshold picked today is wrong next month.
