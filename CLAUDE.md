...
- `deadman/` — host-specific (`Hostname == "egnor-2020"`). A dead-man's switch for the monitoring parent itself. Everything else in this repo alerts *from* egnor-2020, so egnor-2020's own death is the one outage it cannot report: netdata, the alert evaluator, postfix, and several monitored sites all go at once. `netdata-deadman.timer` fires `/usr/local/sbin/netdata-deadman` every 5 minutes; the script pings a healthchecks.io check URL, but only after local netdata answers `http://127.0.0.1:19998/api/v1/info` — so the switch covers netdata-dead and netdata-wedged, not just host-down (on a bad netdata it pings `<url>/fail` for an immediate alert instead of waiting out the grace period). healthchecks.io notifies the same `ntfy.sh/eacs-alerts` topic netdata uses, so everything still lands in one place. `HC_PING_URL` lives out-of-band in `/etc/default/netdata-deadman` (a forgeable-ping secret, like the ntfy token); if it is unset the script exits non-zero on purpose, so netdata's `systemdunits` alert catches a deadman that was never configured.
