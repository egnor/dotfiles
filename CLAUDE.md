...
| the probe | HTTP GET of the ping URL, but only after local netdata answers `http://127.0.0.1:19998/api/v1/info`; each step retries 3× so a netdata restart or a passing hc-ping.com 502 doesn't fail the unit | a mail message: `sendmail -f mail-deadman@eacs.io` to `<uuid>@hc-ping.com` |
